...
class ToReadItem(BaseItem):
    def __init__(self, title, url):
        super(ToReadItem, self).__init__(title)
        self.url = url

    def __str__(self):
        return 'ToRead: {} for {}'.format(self.heading, self.url)

    @classmethod
    def construct(cls):
        input_function = get_input_function()
        title = input_function('Input book or article title:')
        url = input_function('Input book URL:')
